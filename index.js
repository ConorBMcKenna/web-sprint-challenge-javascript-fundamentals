...
const zooAnimals = [
  ...
  */
  ...
  function animalNames(array){
    let acc = [];
    array.forEach(animal => acc.push(`name: ${animal.animal_name}, scientific: ${animal.scientific_name}`) );
    return acc;
  }
  ...
  */
  ...
  function lowerCaseNames(array){
    return array.map(animal => animal.animal_name.toLowerCase());
  }
  ...
  */
  ...
  function USApop(array){
    return array.reduce((acc,animal)=>acc + animal.population,0);
  }
...
function greeting(firstName, lastName){
   return consume(firstName, lastName, (firstName, lastName)=>`Hello ${firstName} ${lastName}, nice to meet you!`);
  }
...
function CuboidMaker({length, height, width}){
  this.length=length;
  this.width=width;
  this.height=height; 
}
CuboidMaker.prototype.constructor=CuboidMaker;
